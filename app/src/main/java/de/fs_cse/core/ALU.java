...
public class ALU {
    ...
    private void arithmeticShiftLeft(long shiftBy, long value, int regId){
        ...
        write(regId, z);
        UnsignedLong uLongShiftBy = UnsignedLong.fromLongBits(shiftBy);
        UnsignedLong uLong64 = UnsignedLong.fromLongBits(64);
        if(uLongShiftBy.compareTo(uLong64) < 0){
    ...
    private void add(long x, long y, int regId){
        ...
        UnsignedLong uLongX = UnsignedLong.fromLongBits(x);
        UnsignedLong uLongY = UnsignedLong.fromLongBits(y);
        UnsignedLong uLongZ = UnsignedLong.fromLongBits(z);

        zf = z == 0;
        cf = uLongZ.compareTo(uLongX) < 0 || uLongZ.compareTo(uLongY) < 0;
