...
public class ULM {
    ...
    public long peekQuad(long address){
        return cpu.memory.peek(address, 8);
    }
    ...
    public boolean step() {
        ...
        for(ObserverULM observer: observers) observer.flagCallback(cpu.alu.zf, cpu.alu.of, cpu.alu.cf, cpu.alu.sf);
    ...
    public void loadProgram(int[] program) {
        byte[] bprogram = new byte[4*program.length];
        for(int i = 0; i < program.length; i++){
            bprogram[4*i+0] = (byte) (program[i] >> 24);
            bprogram[4*i+1] = (byte) (program[i] >> 16);
            bprogram[4*i+2] = (byte) (program[i] >> 8);
            bprogram[4*i+3] = (byte) (program[i] >> 0);
        }
        loadProgram(bprogram);
    }
    ...
    private void analyseNextInstruction() {
        ...
        for (ObserverULM observer : observers) observer.nextInstruction(cpu.ip, nextOpfield, disassembly);
    }
    ...
    private void rPutc(OperationField opfield) {
        io.putc((byte) cpu.alu.read(opfield.rX)); //cast by cutoff
    }

    private void uPutc(OperationField opfield) {
        io.putc((byte) opfield.uX); //cast by cutoff
    }
    ...
    private void initDisassemblies() {
        ...
        //IO
        disassemblies[0x60] = (opfield) -> format_R("getc", opfield.rX);
        disassemblies[0x61] = (opfield) -> format_R("putc", opfield.rX);
        disassemblies[0x69] = (opfield) -> String.format("%s '%s'", "putc", disassembleChar((char) opfield.uX));
        //NOP
    ...
    private static String disassembleChar(char c){
        switch (c){
            case '\n': return "\\n";
            case '\t': return "\\t";
            case '\b': return "\\b";
